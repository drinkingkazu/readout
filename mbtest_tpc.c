...
static int check_status(WDC_DEVICE_HANDLE hDevTrigger,
...
static void MenuMBtest(WDC_DEVICE_HANDLE hDev,  // Valid PCIE for TPC crate + Trigger
		       WDC_DEVICE_HANDLE hDev1, // Valid PCIE for TPC Neutrino xmit
		       WDC_DEVICE_HANDLE hDev2) // Valid PCIE for TPC Supernova
{
  ...
  */
  ...
  case 7:
    ...
    */
	...
	//scanf("%d",&ik);
	usleep(1000);
	ik=tpc_adc_setup(hDev, imod_fem, iframe, itpc_adc, ihuff, icom_factor, timesize);
      }
    ...
    if(itpc_read == 1) {
      ...
      ik = pmt_link_setup(hDev,imod_xmit_tpc,imod_st_tpc,0);      // no supernova
      //
      //
      //
      //
    ...
    if(itpc_read == 1) {
      ...
      dwStatus = WDC_DMAContigBufLock(hDev1, &pbuf_rec_n1_tpc, dwOptions_rec, dwDMABufSize_tpc, &pDma_rec_n1_tpc);
      if (WD_STATUS_SUCCESS != dwStatus) {
	...
	printf(" buffer allocation 1 higher address = %x\n", u32Data);
      ...
      dwStatus = WDC_DMAContigBufLock(hDev1, &pbuf_rec_n2_tpc, dwOptions_rec, dwDMABufSize_tpc, &pDma_rec_n2_tpc);
      if (WD_STATUS_SUCCESS != dwStatus) {
	...
	dwStatus = WDC_DMAContigBufLock(hDev2, &pbuf_rec_s1_tpc, dwOptions_rec, dwDMABufSize_tpc, &pDma_rec_s1_tpc);
	if (WD_STATUS_SUCCESS != dwStatus) {
	...
	dwStatus = WDC_DMAContigBufLock(hDev2, &pbuf_rec_s2_tpc, dwOptions_rec, dwDMABufSize_tpc, &pDma_rec_s2_tpc);
	if (WD_STATUS_SUCCESS != dwStatus) {
	...
	dwStatus = WDC_DMAContigBufLock(hDev2, &pbuf_rec_s3_tpc, dwOptions_rec, dwDMABufSize_tpc, &pDma_rec_s3_tpc);
	if (WD_STATUS_SUCCESS != dwStatus) {
	...
	dwStatus = WDC_DMAContigBufLock(hDev2, &pbuf_rec_s4_tpc, dwOptions_rec, dwDMABufSize_tpc, &pDma_rec_s4_tpc);
	if (WD_STATUS_SUCCESS != dwStatus) {
    ...
    if(check_status(hDev, hDev, imod_trig, imod_xmit_tpc+1, imod_st_tpc, imod_xmit_tpc))
      break;
    ...
    check_status(hDev, hDev, imod_trig, imod_xmit_tpc+1, imod_st_tpc, imod_xmit_tpc);
    ...
    */
	...
	if(check_status(hDev, hDev, imod_trig, imod_xmit_tpc+1, imod_st_tpc, imod_xmit_tpc) && respect_status )
	    break;
      }
	...
	if(key == 'q') break;
      ...
      //
      //
      if(itrig_pulse == 1 && kazu_ctr%50==0) {
	imod = imod_trig;
	buf_send[0]=(imod<<11)+(mb_trig_p1_delay)+((p1_delay)<<16); // set up p1 delay;
	i=1;
	k=1;
...
#define mb_trig_rd_status 32
  ...
  // Status
  if(sum_status_tpc_fem || sum_status_tpc_xmit) {
...
#define  dma_abort 0x2
  ...
  //printf("\033[95miold=0x%x ... i=0x%x\033[00m\n",iold,i);
  //
  //
  while ((iold - i) >= 16) {
    dwAddrSpace =cs_bar;
...
#define  dma_buffer_size        10000000
  ...
  */
  ...
  threshold1 = 10;   // 150 discriminator threshold 1
  ...
  buf_send[0]=(imod<<11)+(ichip<<8)+mb_feb_pmt_trig_delay+(0x9<<16);  
  i=1;
  k=1;
  i = pcie_send(hDev, i, k, px);
  ...
  buf_send[0]=(imod<<11)+(ichip<<8)+mb_feb_pmt_trig1_delay+(0x9<<16);  
  i=1;
  k=1;
  i = pcie_send(hDev, i, k, px);
